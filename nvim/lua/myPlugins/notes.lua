...
-- TODO: Templates
-- TODO: Better goto file. If the cursor is inside () then follow the path insede () not just the word under the cursor
-- TODO: Better goto file. If the cursor is inside [] then check if () at the end and follow the link inside ()
-- TODO: When searching for a note, add the ability that if none are found to create a new note.
...
-- TODO: Chnage this so that the user does not have to know the exact name of the file, but can also search the aliases.
...
local function note_alias_picker(opts)
	opts = opts or {}

	local notes = vim.fn.glob(M.opts.notes_folder .. "/**/*.md", true, true)
	local results = {}

	for _, note in ipairs(notes) do
		for _, alias in ipairs(get_aliases(note)) do
			results[#results + 1] = {
				display = alias,
				ordinal = alias,
				path = note,
	...
	pickers
		.new(opts, {
			prompt_title = "Notes and aliases",
...
if builtin_ok then
	...
	vim.keymap.set("n", "<Leader>na", note_alias_picker)
end
